...
const AnnualBarChart: React.FC<AnnualBarChartProps> = ({ stationId }) => {
  ...
  const parameterOptions = [
    { value: "discharge", label: "Annual Mean Discharge (m³/s)" },
  ];
  ...
  const chartData = useMemo(() => {
    ...
    return data.map((yearData: any) => {
      ...
      const annualMean = monthlyValues.reduce((sum, val) => sum + val, 0) / 12;
  ...
  return (
    <Card className={classes.chartContainer}>
      <Title level={4}>Annual Mean Discharge</Title>
      ...
      <div className={classes.chart}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <YAxis
              label={{
                value: "Annual Mean Discharge (m³/s)",
                angle: -90,
                position: "insideLeft",
              }}
            />
